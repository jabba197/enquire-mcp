...
| Tool | Kind | Gating | Summary |
|---|---|---|---|
| `obsidian_reindex` | read | `--persistent-index` | Force an immediate FTS5 refresh after out-of-band edits, and verify specific notes against the index (`verify_paths` / `since`). |
...
| `--vault-label <label>` | none (`ENQUIRE_VAULT_LABEL`) | Short human name of the vault this server serves (e.g. `"QVC (work)"`). Prefixed onto every tool description as `[Vault: <label>]` and appended to each tool title, so a client connected to several Obsidian MCP servers can tell identically-named tool sets apart — and is far less likely to read or write the wrong vault. Falls back to the `ENQUIRE_VAULT_LABEL` env var. |
