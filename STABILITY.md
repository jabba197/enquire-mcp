...
46 tools total = **34 always-on read** + **1 opt-in via `--persistent-index`** + **1 opt-in via `--persistent-index` + `--diagnostic-search-tools`** + **3 opt-in via `--diagnostic-search-tools`** + **7 gated by `--enable-write`**. Names + argument shapes are stable in v3.x.
...
**Read — opt-in via `--persistent-index` (1):** `obsidian_reindex`. Forces an immediate FTS5 refresh after out-of-band edits, and verifies specific notes against the index.
